...
namespace LightFeather.Ui.Pages;
...
public class IndexModel(ILogger<IndexModel> logger,IHttpClientFactory clientFactory) : PageModel
{
    ...
    private async Task PopulateSupervisorSelection(HttpClient client)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            Supervisors = supervisorStringValues?.Select(supervisorStringValue =>
            {
                return new SelectionSupervisorDto { SupervisorString = supervisorStringValue };
            }).ToList() ?? [];
